...
export default function Wishlist() {
  ...
  const handleRemove = async (productId) => {
    try {
      await axios.delete(`/remove-from-wishlist/${Id}/${productId}`); // DELETE request
      toast.success('Product removed from wishlist');
      fetchWishlist(); // Re-fetch the updated wishlist
    } catch (error) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      {products.length === 0 ? (
        // If wishlist is empty, show "Continue Shopping" button
        <div className="text-center mt-20">
          <h2 className="text-2xl font-semibold text-gray-700 mb-4">Your Wishlist is Empty</h2>
          <button
            onClick={() => navigate('/products')} // Replace with your shop page route
        ...
        // If wishlist has items, show the product cards
      ...
      {products.length > 0 && (
        <div className="fixed bottom-4 right-4">
          <button className="px-6 py-2 bg-black text-white rounded-full hover:bg-black/90 transition-colors"
