...
export default function UserList() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      const response = await axios.get('/userlist', { params: { page: currentPage } });
  ...
  const toggleStatusMutation = useMutation({
    mutationFn: (customerId) => axios.patch(`/admin/toggleStatus/${customerId}`),
    onSuccess: () => {
      ...
      <div className="bg-white rounded-lg shadow-sm">
                ...
                <tr key={customer._id} className="border-b last:border-b-0">
                  ...
                  <td className="px-6 py-4">
                    <label className="relative inline-flex items-center cursor-pointer">
                      ...
                      />
                      <div className={`w-11 h-6 ${customer.status ? 'bg-green-500' : 'bg-red-500'}`}></div>
                    </label>
                  </td>
                  <td className="px-6 py-4">
                    <button onClick={() => handleDeleteClick(customer)} className="text-gray-600 hover:text-red-600 transition-colors">
        ...
        <div className="p-4 flex justify-center items-center space-x-2">
          ...
          {[...Array(Math.ceil(totalCustomers / customersPerPage))].map((_, index) => (
            <button key={index} onClick={() => setCurrentPage(index + 1)} className={currentPage === index + 1 ? 'bg-black text-white' : ''}>
              {index + 1}
            </button>
          ))}
          <button onClick={() => setCurrentPage((prev) => prev + 1)} disabled={currentPage === Math.ceil(totalCustomers / customersPerPage)}>
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-6 rounded shadow-lg text-center">
            ...
            <div className="mt-4 flex justify-center space-x-4">
              ...
              <button onClick={cancelDelete} className="px-4 py-2 bg-gray-300 text-gray-700 rounded">Cancel</button>
            </div>
          </div>
        </div>
      )}
