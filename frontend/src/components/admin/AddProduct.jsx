...
export default function AddProduct() {
  ...
  const handleCrop = () => {
    if (cropperRefs.current[imageBeingCropped === 'main' ? 0 : parseInt(imageBeingCropped.replace('thumbnail', ''))]) {
      const cropperInstance = cropperRefs.current[imageBeingCropped === 'main' ? 0 : parseInt(imageBeingCropped.replace('thumbnail', ''))].cropper;
      if (cropperInstance) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      try {
        ...
        const response = await axios.post("/addproduct", formDataToSubmit, 
          );
        ...
        console.error("Error during image upload or backend submission:", error);
      }
    }
  };
                ...
                <div key={index} className="relative">
                  {thumbnail ? (
                    ...
                    <div className="text-center text-sm text-gray-500">
                      ...
                      <button
                        type="button"
                        onClick={() => document.getElementById(`thumbnail-${index + 1}-upload`).click()}
                        className="text-blue-500 font-semibold"
                      >
                        Select File
                      </button>
                  ...
                  <input
                    ...
                    type="file"
                    accept="image/*"
                    onChange={(e) => handleImageDrop(e, `thumbnail${index + 1}`)}
                    className="hidden"
                  />
                </div>
              ))}
        ...
        {isCropping && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
            <div className="bg-white p-6 rounded-lg">
              ...
              <Cropper
                src={imageBeingCropped === 'main' ? mainImage : thumbnails[parseInt(imageBeingCropped.replace('thumbnail', '')) - 1]}
                ref={(el) => cropperRefs.current[imageBeingCropped === 'main' ? 0 : parseInt(imageBeingCropped.replace('thumbnail', ''))] = el}
